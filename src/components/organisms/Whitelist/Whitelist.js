...
const PoolViewItem = ({ address, symbol, asset, decimals: pool_decimals, waiting, blacklisted, votes_vp, vp, asset_key, group_key, ...rest }) => {
  ...
  const whitelistedUrl = generateLink({ amount: 1e4, data: { vote_whitelist: 1, pool_asset: asset }, aa: appConfig.AA_ADDRESS, from_address: walletAddress, is_single: !walletAddress });
  const blacklistedUrl = generateLink({ amount: 1e4, data: { vote_blacklist: 1, pool_asset: asset }, aa: appConfig.AA_ADDRESS, from_address: walletAddress, is_single: !walletAddress });
